...
def get_db_path(dbname):
    ...
    db_path = os.path.join(base_name, 'SQLitedb', dbname)
    return db_path
...
def check_date():
    ...
    path = get_db_path('registration_form.db')
...
def create_home_frame(home):
    ...
    def get_visitors_count():
        # get the no. of members from the database
        path = get_db_path('visitors_log.db')
        conn=sqlite3.connect(path)
        cursor=conn.cursor()
    ...
    def get_employee_count():
        # get the no. of members from the database
        path = get_db_path('register_employee.db')
        conn=sqlite3.connect(path)
        cursor=conn.cursor()
    ...
    def get_trainer_count():
        # get the no. of members from the database
        path = get_db_path('register_trainer.db')
        conn=sqlite3.connect(path)
        cursor=conn.cursor()
    ...
    def get_gym_equipment_count():
        # get the no. of members from the database
        path = get_db_path('register_equipment.db')
        conn=sqlite3.connect(path)
        cursor=conn.cursor()
...
def update_income_report(root, ax, canvas):
    ...
    path = get_db_path('registration_form.db')
    conn_members=sqlite3.connect(path)
...
def update_visitors_income_report(root, ax, canvas):
    ...
    # Connect to the members database
    path = get_db_path('visitors_log.db')
    conn_visitors=sqlite3.connect(path)
...
class ScanFrame(ctk.CTkFrame):
    ...
    def record_attendance(self, member_data, attendance_type):
        try:
            ...
            path = get_db_path('registration_form.db')
            with sqlite3.connect(path) as conn_registration:
            ...
            path = get_db_path('registration_form.db')
            with sqlite3.connect(path) as conn_attendance:
...
class RecordsFrame(ctk.CTkFrame):
    ...
    def create_ui_elements(self):
        ...
        path = get_db_path('attendance_records.db')
        conn=sqlite3.connect(path)
        cursor=conn.cursor()
        cursor.execute('''
        ...
        try:
            ...
            with sqlite3.connect(path) as conn:
                ...
                if status and status[0] in ['Active', 'On Leave']:
                    ...
                    path = get_db_path('trainer_attendance_records.db')
                    with sqlite3.connect(path) as attendance_conn:
    ...
    def record_attendance(member_data, attendance_type):
        try:
            ...
            with sqlite3.connect(path) as conn:
                ...
                if status and status[0] in ['Active', 'On Leave']:
                    # Only proceed if the status is 'Active' or 'On Leave'
                    path = get_db_path('employee_attendance_records.db')
                    with sqlite3.connect(path) as attendance_conn:
                        attendance_cursor=attendance_conn.cursor()
...
def create_account_management_frame(master):
...
def register(full_name, contact_no, username, password, contact_no_entry, full_name_entry, username_entry,
    ...
    path = get_db_path('registered_users.db')
    conn=sqlite3.connect(path)
    cursor=conn.cursor()
...
def create_login_window():
    try:
        ...
        background_image_path = os.path.join(base_path, 'templates', 'pat.png')
        ...
        logo_image_path = os.path.join(base_path, 'templates', 'gym_dark.png')
